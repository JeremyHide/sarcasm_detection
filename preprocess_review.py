...
# Remove the id of the review.
def removeID(review):
    newList = []
    if '\t' in review:
        new_review = review.split('\t')[1]
    return new_review

# Remove the head of the review.
def removeHead(review):
    new_review = review.split('<REVIEW>')[1]
    new_review_v2 = new_review.replace('</REVIEW>','').replace('\n','')+'\n'
    print(new_review_v2)
    return new_review_v2
